...
class TestImageSize:
    ...
    def test_image_size_resolutions(self):
        """Test ImageSize resolution property."""
        assert ImageSize.LITTLE.resolution == 480
        assert ImageSize.SMALL.resolution == 1080
        assert ImageSize.LARGE.resolution == 2048
        assert ImageSize.HUGE.resolution == 4096
        assert ImageSize.ORIGINAL.resolution == 0

    def test_size_resolutions_mapping(self):
        """Test SIZE_RESOLUTIONS dict matches enum."""
        assert SIZE_RESOLUTIONS[ImageSize.LITTLE] == 480
        assert SIZE_RESOLUTIONS[ImageSize.SMALL] == 1080
        assert SIZE_RESOLUTIONS[ImageSize.LARGE] == 2048
        assert SIZE_RESOLUTIONS[ImageSize.HUGE] == 4096
        assert SIZE_RESOLUTIONS[ImageSize.ORIGINAL] == 0

    def test_image_size_from_string(self):
        """Test creating ImageSize from string."""
        assert ImageSize("little") == ImageSize.LITTLE
        assert ImageSize("small") == ImageSize.SMALL
        assert ImageSize("large") == ImageSize.LARGE
        assert ImageSize("huge") == ImageSize.HUGE
        assert ImageSize("original") == ImageSize.ORIGINAL

    def test_all_sizes_have_resolutions(self):
        """Test that all ImageSize values have resolution mappings."""
        for size in ImageSize:
            assert size in SIZE_RESOLUTIONS
            assert isinstance(size.resolution, int)
